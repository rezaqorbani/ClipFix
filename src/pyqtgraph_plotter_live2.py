...
class LiveAudio():
    def __init__(self, nchannels, curves):
        ...
        self.input_data = np.zeros((self.rate * self.number_of_seconds, self.nchannels))
        self.output_data = np.zeros(1)

        # Set up plot length
        self.plot_length = 1 # set the plot length to 5 seconds
    ...
    def update(self):
        ...
        if self.recording:
            ...
            for i in range(self.nchannels):
                ...
                self.input_data[len(inputs)*self.n:len(inputs)*(self.n+1), i] = inputs[:, i]
           ...
           # self.input_data = np.concatenate((self.input_data, inputs))
            ...
            for i in range(self.nchannels):
                ...
                self.curves["input"][i].setData(time_array[:, ], self.input_data[:, i])
            ...
            if self.nchannels != 1:
                ...
                self.curves["output"][0].setData(time_array[:, ], self.output_data[:, ])
            else:
                self.curves["output"][0].setData(time_array[-self.plot_length * self.rate:, ],
